function [P,Uinit,output,fit,obj_val] = cp_als_orth(X,R,varargin)
%CP_ALS Compute a CP decomposition of any type of tensor.
%
%   P = CP_ALS_ORTH(X,R) computes an estimate of the best rank-R
%   CP model of a tensor X using the Orth-ALS algorithm
...
%   orthogonalized at every iteration. This is incontrast to Hybrid-ALS,
%   where orthogonalization is only performed upto a specified number of 
%   steps, after which standard ALS is run. 
%   
...
%
%   P = CP_ALS_ORTH(X,R,'param',value,...) specifies optional parameters and
...
%
%   X_cpd = cp_als_orth(X_tensor, 30, 'orth', 1);
%
%   [P,U0] = CP_ALS_ORTH(...) also returns the initial guess.
%
%   [P,U0,out] = CP_ALS_ORTH(...) also returns additional output that contains
...
%   P = cp_als_orth(X,2);
%   P = cp_als_orth(X,2,'dimorder',[3 2 1]);
%   P = cp_als_orth(X,2,'dimorder',[3 2 1],'init','nvecs');
%   U0 = {rand(5,2),rand(4,2),[]}; %<-- Initial guess for factors of P
%   [P,U0,out] = cp_als_orth(X,2,'dimorder',[3 2 1],'init',U0);
